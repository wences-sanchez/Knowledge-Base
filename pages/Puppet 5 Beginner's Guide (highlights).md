...
- Highlights first synced by [[Readwise]] [[Friday, 28-10-2022]]
	...
	- Using templates in your manifests
		...
		- # Manage NTP
		  ...
		  ensure_packages(['ntp'],
			...
			- **Note**: Example of class definition in Puppet
		- The following example shows what these markers look like (aws_credentials.epp):
		  ...
		  Everything outside the <%= and %> delimiters is literal text and will be rendered as-is by Puppet.
			- **Note**: Example of syntax of templates in Puppet
	...
	- Services
		...
		- You can see that instead of the class keyword, we use the define keyword. This tells Puppet that we are creating a defined resource type instead of a class. The type is called user_with_key, and once it's defined, we can declare as many instances of it as we want, just like any other Puppet resource:
			- **Note**: Differences between class and defined resource in Puppet
		- called declarative style
	...
	- Distributing Puppet manifests
		...
		- We can summarize the rules by saying that nodes should only include roles, and roles should only include profiles.
	...
	- Types of Hiera data
		- example:
		  ...
		  When you call lookup('cobbler_config', Hash) in a manifest, the data will be returned as a Puppet hash, and you can reference individual values in it using the normal Puppet hash syntax
			- **Note**: Also with dot notation usage
	- The hierarchy
	...
	- Creating resources with Hiera data
		...
		- When we call each on this hash, we specify two parameters to the loop instead of one:
		  ...
		  * => $attrs,
		  ...
		  You may recall from the previous chapter that the * operator (the attribute splat operator) tells Puppet to treat $attrs as a hash of attribute-value pairs.
	- 7. Mastering modules
		- r10k is the de facto standard module manager for Puppet deployments, and we'll be using it to manage modules throughout the rest of this book
			- **Note**: What is r10k in Puppet?
		- you can use the generate-puppetfile tool to find out what dependencies you need so that you can add them to your Puppetfile.
	...
	- Exploring the standard library
		- When you declare the same package resource in more than one place, Puppet will give an error message and refuse to run. If the package is declared by ensure_packages(), however, Puppet will run successfully.
		...
		- The dirname() function is very useful if you have a string path to a file or directory and you want to reference its parent directory, for example to declare it as a Puppet resource (dirname.pp):
		  ...
		  notice(dirname($file))
		- With the pry gem installed in Puppet's context, you can call pry() at any point in your code. When you apply the manifest, Puppet will start an interactive Pry shell at the point where the pry() function is called. You can then run the catalog command to inspect Puppet's catalog
	- Writing your own modules
		- puppet:///modules/pbg_ntp/ntp.conf
		  We haven't seen this kind of file source before, and it's generally only used within module code. The puppet:// prefix indicates that the file comes from within the Puppet repo, and the path /modules/pbg_ntp/ tells Puppet to look within the pbg_ntp module for it.
