...
- Highlights first synced by [[Readwise]] [[Friday, 28-10-2022]]
	...
	- 11 TESTING YOUR CODE
		...
		- print(bicycles[-1])
			- **Note**: Show the last item of the list bicycles.
		- popped_motorcycle = motorcycles.pop()
		  ➌ print(motorcycles)
		  ➍ print(popped_motorcycle)
			- **Note**: Remove the last item of the list motorcycles, 
			  then show the resultant list, 
			  then show the removed object
			  [call it removed_item]
		- first_owned = motorcycles.pop(0)
			- **Note**: Save in a variable named first_owned the (just removed) element with index 0 of the list motorcycles
		- To write a test case for a function, import the unittest module and the function you want to test. Then create a class that inherits from unittest.TestCase, and write a series of methods to test
			- **Note**: Steps to create tests in Python
	...
	- 9 CLASSES
		- Any method that starts with test_ will be run automatically when we run test_name_function.py.
			- **Note**: How do we call a testing function?
		- message = f"My first bicycle was a {bicycles[0].title()}."
			- **Note**: Show a variable named status inside a string (in Python 3)
		- motorcycles.remove('ducati')
			- **Note**: 'Steal' the element with value 'ducati' from the motorcycles list
		...
		- motorcycles.append('ducati')
			- **Note**: Add 'ducati' at the end of the list motorcycles
		- motorcycles.remove(too_expensive)
			- **Note**: 'Steal' the element with content 'ducati' from the motorcycles list.
			  [Use the already defined variable named my_brand for this task]
		- len(cars)
			- **Note**: Function to get the size of the list cars
		- def __init__(self, make, model, year):
		           """Initialize attributes of the parent class."""
		  ➍         super().__init__(make, model, year)
			- **Note**: How would you define a constructor of a child class in Python?
		- A method is an action that Python can perform on a piece of data.
		- bicycles = ['trek', 'cannondale', 'redline', 'specialized']
			- **Note**: Construct a list named bicycles which contains the items:
			  * 'trek'
			  * 'cannondale'
			  * 'redline'
			  * 'specialized'
		- print(bicycles[0])
			- **Note**: Show the first element of the list bicycles.
		- motorcycles.insert(0, 'ducati')
			- **Note**: Insert 'ducati' at the beginning of the list motorcycles without using square brackets:
		- cars.sort()
			- **Note**: Order alphabetically a list named cars. Make this change permanent.
		- hese strings are called f-strings. The f is for format, because Python formats the string by replacing the name of any variable in braces with its value.
		- del motorcycles[0]
			- **Note**: Remove the first element of the list motorcycles
		- cars.sort(reverse=True)
			- **Note**: Order alphabetically inversed a list named cars. Make this change permanent.
		...
		- print(sorted(cars))
			- **Note**: Show an alphabetically ordered list named cars. Make this change temporal.
		- Sometimes you won't know the position of the value you want to remove from a list. If you only know the value of the item you want to remove, you can use the
		- Python programmers use all capital letters to indicate a variable should be treated as a constant and never be changed
		- cars.reverse()
			- **Note**: Make the order of the list cars inverse.
		- Although you can't modify a tuple, you can assign a new value to a variable that represents a tuple.
			- **Note**: Is it possible to reassign a tuple's content?
		- Python considers the first item in a list to be at position 0, not position 1
		- for value in range(1, 5):
		    print(value)
			- **Note**: Show the numbers: [1, 4]
		...
		- print(players[0:3])
			- **Note**: Show in a slice the first three items of the list players.
		- friend_foods = my_foods[:]
			- **Note**: Copy the list my_foods to the list friend_foods without affectations.
		...
		- print(players[1:4])
			- **Note**: Show in a slice the second, third and fourth items of the list players
		- alphabetically
		- numbers = list(range(1, 6))
			- **Note**: Construct a list with the numbers: [1, 5]
		...
		- For dictionaries, specifically, you can use the get() method to set a default value that will be returned if the requested key doesn’t exist.
			- **Note**: Another way of access to an element in a dict instead of indexes in square brackets
		- The PEP 8 guidelines for line length are not set in stone, and some teams prefer a 99-character limit.
		- keep in mind when writing your own for loops that you can choose any name you want for the temporary variable that will be associated with each value in the list. However, it’s helpful to choose a meaningful name that represents a single item from the list.
		- print(f"You ordered a {pizza['crust']}-crust pizza "
		       "with the following toppings:")
			- **Note**: How can you split a string in multiple lines?
	...
	- 6 DICTIONARIES
		...
		- alien_0 = {'color': 'green', 'points': 5}
			- **Note**: Build a dict named alien_0 with:
			  color ---> green
			  points ---> 50
	...
	- 8 FUNCTIONS
		...
		- The best approach is to import the function or functions you want, or import the entire module and use the dot notation. This leads to clear code that's easy to read and understand.
			- **Note**: How should I use the import's in Python?
	- 10 FILES AND EXCEPTIONS
		...
		- u'll often want to run through all entries in a list, performing the same task with each item. For example, in a game you might want to move every element on the screen by the same amount, or in a list of numbers you might want to perform the same statistic
		- Although you can’t modify a tuple, you can assign a new value to a variable that represents a tuple.
			- **Note**: Is it possible to reassign a tuple's content?
		- super().__init__(make, model, year)
			- **Note**: How do you call a base class' constructor?
		- The best approach is to import the function or functions you want, or import the entire module and use the dot notation. This leads to clear code that’s easy to read and understand.
			- **Note**: How should I use the import's in Python?
		- alien_0 = {'color': 'green', 'points': 5}
			- **Note**: Build a dict named alien_0 with:
		...
		- ➊ alien_0['x_position'] = 0
		  ➋ alien_0['y_position'] = 25
			- **Note**: Add a new key-value entry to an existing dict in Python.
			  Values x-0, y-25.
			  
			  BEFORE:
			  my_dict = {'color': 'green', 'points': 5}
		- del alien_0['points']
			- **Note**: Remove the entry with key ‘points’ from the dict alien.
		- point_value = alien_0.get('points', 'No point value assigned.')
			- **Note**: Access to a dict but printing a message in case of error.
			  dict: my_map
			  key: ‘point’
			  Message: ‘unassigned’
			  Variable: point_value
		- for key, value in user_0.items():
			- **Note**: Loop over dict user_0 with its separate parts
		- for name in favorite_languages.keys():
		       print(name.title())
			- **Note**: Iterate over the keys of a dict fav_langs {name:value}
		- The keys() method isn’t just for looping: it actually returns a list of all the keys, and the line at ➊ simply checks if 'erin' is in this list.
			- **Note**: Tell another use of keys() in Python aside looping
		- for language in favorite_languages.values():
			- **Note**: Iterate over the content of a dict fav_langs on a loop {key: content}
		- for language in set(favorite_languages.values()):
			- **Note**: Iterate over the content of a dict without repeating.
			  Dict: fav_langs
		- for alien_number in range(30):
			- **Note**: Repeat a message ‘me’ 30 times in Python.
		- The function describe_pet() hasn’t changed. But when we call the function, we explicitly tell Python which parameter each argument should be matched with. When Python reads the function call, it knows to assign the argument 'hamster' to the parameter animal_type and the argument 'harry' to pet_name. The output correctly shows that we have a hamster named Harry.
		  ...
		  The order of keyword arguments doesn’t matter because Python knows where each value should go.
			- **Note**: What are the differences between:
			  
			  describe_pet(animal_type='hamster', pet_name='harry')
			  
			  describe_pet(‘hamster', ‘harry')
		- print_models(unprinted_designs[:], completed_models)
			- **Note**: Print via a function in Python a list without being able to modify it.
		- def make_pizza(*toppings):
		    """Print the list of toppings that have been requested."""
		    print(toppings)
			- **Note**: Design a function which contains unlimited arguments.
		- The asterisk in the parameter name *toppings tells Python to make an empty tuple called toppings and pack whatever values it receives into this tuple. The print() call in the function body produces output showing that Python can handle a function call with one value and a call with three values. It treats the different calls similarly. Note that Python packs the arguments into a tuple, even if the function receives only one value
			- **Note**: Explain how unlimited function parameters works in Python
		- def build_profile(first, last, **user_info):
		       """Build a dictionary containing everything we know about a user."""
		  ➊     user_info['first_name'] = first
		       user_info['last_name'] = last
		       return user_info
			- **Note**: Can you pass infinite key-value entries to a function in Python?
		- import pizza
			- **Note**: Bring the module ‘pizza’ into your scope to use pizza.make_pizza()
		- from pizza import make_pizza
			- **Note**: Bring the module ‘pizza’ into your scope to use ONLY pizza.make_pizza().
			  
			  Use it.
		- from pizza import make_pizza as mp
			- **Note**: Bring the module ‘pizza’ into your scope to use ONLY pizza.make_pizza(). Use an alias.
			  
			  Use it.
		- import pizza as p
			- **Note**: Bring a module ‘pizza’ to your scope but using an abbreviation.
		...
		- def __init__(self, name, age):
		  ...
		  ➍         self.name = name
		           self.age = age
			- **Note**: Build a constructor in Python
		- def sit(self):
		           """Simulate a dog sitting in response to a command."""
		           print(f"{self.name} is now sitting.")
			- **Note**: Define a method in Python
		- my_dog = Dog('Willie', 6)
			- **Note**: Construct an object my_dog by calling the constructor Dog with the arguments:
		- def __init__(self, make, model, year):
		           """Initialize attributes of the parent class."""
		  ➍         super().__init__(make, model, year)
			- **Note**: How do you call a base class' constructor from the subclass?
		- with open('pi_digits.txt') as file_object:
		    contents = file_object.read()
		  print(contents)
			- **Note**: How do you read a text file?
		- Notice how we call open() in this program but not close(). You could open and close the file by calling open() and close(), but if a bug in your program prevents the close() method from being executed, the file may never close. This may seem trivial, but improperly closed files can cause data to be lost or corrupted. And if you call close() too early in your program, you’ll find yourself trying to work with a closed file (a file you can’t access), which leads to more errors. It’s not always easy to know exactly when you should close a file, but with the structure shown here, Python will figure that out for you. All you have to do is open the file and work with it as desired, trusting that Python will close it automatically when the with block finishes execution.
			- **Note**: Should I close a file in Python or not?
		- Windows systems use a backslash (\) instead of a forward slash (/) when displaying file paths, but you can still use forward slashes in your code.
			- **Note**: Forward slashes OR backslashes in Python?
		- filename = 'pi_digits.txt'
		  
		  ➋ with open(filename) as file_object:
		  ➌     for line in file_object:
		          print(line)
			- **Note**: Example to read a file line by line
		- filename = 'programming.txt'
		  
		  ➊ with open(filename, 'w') as file_object:
		  ➋     file_object.write("I love programming.")
			- **Note**: Write some text in a file in Python.
		- You can open a file in read mode ('r'), write mode ('w'), append mode ('a'), or a mode that allows you to read and write to the file ('r+'). If you omit the mode argument, Python opens the file in read-only mode by default.
			- **Note**: Different file modes in Python.
		- Any code that depends on the try block succeeding is added to the else block. In this case if the division operation is successful, we use the else block to print the result
			- **Note**: What does ‘else’ mean in a Exception?
		- try:
		  ...
		  ➌     else:
		           print(answer)
			- **Note**: Example of exception in Python