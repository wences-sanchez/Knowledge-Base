...
- Highlights first synced by [[Readwise]] [[Thursday, 18-08-2022]]
	- Understanding test-driven development
		...
		- -
		- -
		- Thus, one can say that TDD is all about writing better code, and it is just a happy side effect that we end up with a fully automated test suite as an outcome. #ñspace
		- -
	- Using TDD to build a stock alert application
		- -
		- The unittest framework will pick up any method that starts with test #ñspace
		- -
		- -
	...
	- Reorganizing the test code
		...
		- -
		- -
		- from ..stock import Stock #ñspace
		- -
		- -
		- Including a call to unittest.main() works well with individual scripts since it allows us to run the tests by simply executing the file. However, it is not a very scalable solution. If we have hundreds of files, we would like to run all the tests at once, and not have to execute each file individually. #ñspace
		- -
		- -
		- python3 -m unittest #ñspace
		- -
	- 2. Red-Green-Refactor – The TDD Cycle
		- -
		- in TDD, tests are nothing but requirements. Each time we write a test and implement code to make it pass, what we actually do is make the code meet some requirement. Looking at it another way, tests are just executable requirement specifications. #ñspace
		- -
	- Arrange-Act-Assert
		- -
	...
	- Testing for exceptions
		...
		- with self.assertRaises(ValueError):
		            goog.update(datetime(2014, 2, 13), -1) #ñspace
		- -
	- Exploring assert methods
		- -
		- self.assertAlmostEqual(8.4, goog.price, delta=0.0001) #ñspace
		- -
		- -
	...
	- Brittle tests
		- -
		- A test is brittle when a change in the implementation details requires a change in the test cases. Ideally, a test should be testing the interface and not the implementation directly. After all, it is the interface that other units will be using to interact with this unit. #ñspace
		- -
		- -
		- Brittle tests can be worse than no tests, as the maintenance overhead of having to fix ten or twenty tests with every change in the implementation can turn developers away from TDD, increase the amount of frustration, and lead to teams disabling or skipping testing. #ñspace
		- -
		- -
	...
	- Exploring the Rule classes
		- -
		- The all function is a built-in function that takes a list and returns True only if every element of that list is True. #ñspace
		- -