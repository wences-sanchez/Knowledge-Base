...
- Highlights first synced by [[Readwise]] [[Monday, 31-10-2022]]
	- 1 Introduction
		...
		- A native Linux container format that Docker calls libcontainer.
		  ...
		  Interactive shell: You can create a pseudo-tty and attach to STDIN to provide an interactive shell to your container. #flashcard
	...
	- 3 Getting Started with Docker
		...
		- Docker will automatically generate a name at random for each container we create. We see that the container we’ve just created is called gray_cat. If we want to specify a particular container name in place of the automatically generated name, we can do so using the --name flag.
		  ...
		  root@aa3f365f0f4e:/# exit #flashcard
			- How can you name a container?
		- -
		- -
		...
		- $ sudo docker run --name daemon_dave -d ubuntu /bin/sh -c "while true; do echo hello world; sleep 1; done"
		  ...
		  Here, we’ve used the docker run command with the -d flag to tell Docker to detach the container to the background. #flashcard
			- How can you make a daemon of a container?
		- -
		- -
		- see what’s happening. To do so, we can use the docker logs command. The docker logs command fetches the logs of a container.
		  ...
		  $ sudo docker logs -f daemon_dave #flashcard
			- How can you inspect the logs of a container named daemon_dave?
		- -
		- -
		...
		- -
		- -
		- $ sudo docker exec -d daemon_dave touch /etc/new_config_file #flashcard
			- How can you run the command `touch /etc/new_config_file` inside the already-running container named daemon_dave?
		- -
		- -
		- $ sudo docker stop daemon_dave #flashcard
			- How can you terminate a container named daemon_dave?
		- -
		- -
	...
	- 4 Working with Docker images and repositories
		...
		- There are two types of repositories: user repositories, which contain images contributed by Docker users, and top-level repositories, which are controlled by the people behind Docker.
		  ...
		  Alternatively, a top-level repository only has a repository name like ubuntu. The top-level repositories are managed by Docker Inc and by selected vendors who provide curated base images that you can build upon (e.g., the Fedora team provides a fedora image). #flashcard
			- About the two types of repositories in Docker.
		- -
		- -
		- The Dockerfile contains a series of instructions paired with arguments. Each instruction, for example FROM, should be in upper-case and be followed by an argument: FROM ubuntu:18.04. Instructions in the Dockerfile are processed from the top down, so you should order them accordingly. #flashcard
		...
		- -
		- -
		- A Docker image is made up of filesystems layered over each other. At the base is a boot filesystem, bootfs, which resembles the typical Linux/Unix boot filesystem. A Docker user will probably never interact with the boot filesystem. #flashcard
			- What are, basically, Docker images?
		- -
		- -
		- In a more traditional Linux boot, the root filesystem is mounted read-only and then switched to read-write after boot and an integrity check is conducted. In the Docker world, however, the root filesystem stays in read-only mode, and Docker takes advantage of a union mount to add more read-only filesystems onto the root filesystem. A union mount is a mount that allows several filesystems to be mounted at one time but appear to be one filesystem. The union mount overlays the filesystems on top of one another so that the resulting filesystem may contain files and subdirectories from any or all of the underlying filesystems.
		  ...
		  This sounds confusing, so perhaps it is best represented by a diagram. #flashcard
			- How do the tiers work in Docker?
			  [](https://learning.oreilly.com/api/v2/epubs/urn:orm:book:9780988820203/files/media/file2.png)
		- -
		- -
		- The first instruction in a Dockerfile must be FROM. The FROM instruction specifies an existing image that the following instructions will operate on; this image is called the base image. #flashcard
			- What instruction must be the first in a Dockerfile?
		- -
		- -
		...
		- -
		- -
		- $ sudo docker build -t="jamtur01/static_web:v1" . #flashcard
		...
		- -
		- -
		- The WORKDIR instruction provides a way to set the working directory for the container and the ENTRYPOINT and/or CMD to be executed when a container is launched from the image. #flashcard
			- What is WORKDIR used for in Dockerfile?
		- -
		- -
		- The USER instruction specifies a user that the image should be run as; for example:
		  ...
		  This will cause containers created from the image to be run by the nginx user. We can specify a username or a UID and group or GID. Or even a combination #flashcard
			- How could you specify a user in a Dockerfile?
		- -
		- -
		- A volume is a specially designated directory within one or more containers that bypasses the Union File System to provide several useful features for persistent or shared data:
		    ...
		    Volumes persist even if no containers use them. #flashcard
			- Volume features in Docker
		- -
		- -
		- This allows us to add data (like source code), a database, or other content into an image without committing it to the image and allows us to share that data between containers. This can be used to do testing with containers and an application’s code, manage logs, or handle databases inside a container. #flashcard
			- How could you share data between containers?
		- -
		- -
	...
	- 5 Testing with Docker
		...
		- We want to work on and test it simultaneously.
		  ...
		  The -v option works by specifying a directory or mount on the local host separated from the directory on the container with a :. If the container directory doesn’t exist Docker will create it. #flashcard
			- Benefits of volumes in Docker:
		- -
		- -
	...
	- 7 Docker Orchestration and Service Discovery
		...
		- With Docker Compose, we define a set of containers to boot up, and their runtime properties, all defined in a YAML file. Docker Compose calls each of these containers “services” which it defines as:
		  ...
		  A container that interacts with other containers in some way and that has specific runtime properties. #flashcard
